...
* **최종 갱신일**: 2026-08-17
* **문서 버전**: v2.8 (QA03: 미해결 문제점 3건 심층 원인 추적 및 보고 완료)
...
| **현재 버전** | v1.2.0 (Phase 1~5 완료, v1.1 정비 완료, v1.2 피드백 미해결 3건 원인 추적 완료) |
| **테스트** | Vitest 14개 파일 / **79개 케이스 전부 PASS** |
| **타입 검사** | `npx tsc --noEmit` — 0 Errors |
| **git 상태** | `main` 브랜치, 미해결 3건(모드 배경/선택상태, 애니메이션 속도, 4x4 힌트 진동) 원인 분석 완료 (v2.8) |
| **현재 오픈 업무** | PM 재발주 대기 (DEV 신규 파트 3건 제안 — 7.5절 참조) |
...
| v1.2 사용자 피드백 대응 | AI 힌트 맴돔 수정, 애니메이션 속도 통일, 타이틀 카드 배경 통일, 서브패스 배포 | ⚠️ **미해결 3건 재발생 확인 → 원인 분석 완료 및 재발주 대기** (FB-04는 완료) |
...
| :--- | :--- | :--- | :---: | :--- |
| FB-01 | AI 힌트가 4x4/5x5에서 같은 자리 맴돔 (3x3은 정상) | `aiSolver.ts` 4x4 Weighted A* ($w=2$)의 서브옵티멀 지역 루프 및 힌트 화살표 시각 진동 | **DEV03** | 🔄 **재오픈 (7.5절 분석)** |
| FB-02 | 블록 이동 시 상/하/좌/우 애니메이션 속도 불일치 | 종횡비/퍼센트 높이 픽셀 불일치 + 빈 슬롯 동시 역방향 슬라이드 간섭 + 비대칭 이징 | **DEV04** | 🔄 **재오픈 (7.5절 분석)** |
| FB-03 | 홈 화면 4개 모드 카드 배경색 불일치 & 선택 피드백 부재 | CSS shorthand 배경 덮어쓰기 + `selectedMode` 상태/스타일링 UI 부재 | **DEV05** | 🔄 **재오픈 (7.5절 분석)** |
